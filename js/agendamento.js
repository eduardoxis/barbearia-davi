...
import { validarCupom, registrarUsoCupom } from '../routes/cupons.js';

/* ── Estado do cupom ── */
let cupomAplicado = null; // { codigo, desconto, totalFinal, descricao }
...
export function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    el.textContent = d;
    if (isBlocked && !passado) { el.style.textDecoration = 'line-through'; el.style.color = 'rgba(224,32,32,0.4)'; el.title = 'Fechado'; }
...
async function iniciarPagamentoCakto(renovar = false) {
  ...
  const total   = cupomAplicado
    ? cupomAplicado.totalFinal
    ...
    if (d.status === 'aprovado') {
      ...
      try {
        await criarAgendamento({
          ...
          total:        cupomAplicado ? cupomAplicado.totalFinal : cart.reduce((s, c) => s + c.price, 0),
          data:         booking.date,
        ...
        if (cupomAplicado?.codigo) await registrarUsoCupom(cupomAplicado.codigo);
...
function fillConfirm() {
  const totalBruto = cart.reduce((s, c) => s + c.price, 0);
  const totalFinal = cupomAplicado ? cupomAplicado.totalFinal : totalBruto;
  const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
  ...
  set('confTotal',    'R$ ' + totalFinal.toFixed(2));

  // Linha de desconto do cupom
  const cupomRow = document.getElementById('confCupomRow');
  if (cupomRow) {
    if (cupomAplicado) {
      cupomRow.style.display = 'flex';
      const el = document.getElementById('confCupomDesc');
      if (el) el.textContent = `${cupomAplicado.codigo} − R$ ${cupomAplicado.desconto.toFixed(2)}`;
    } else {
      cupomRow.style.display = 'none';
    }
  }

  // Botões de calendar (Feature 15)
  const calWrap = document.getElementById('confCalendarBtns');
  if (calWrap) {
    calWrap.innerHTML = `
      <a href="${gerarLinkGoogleCalendar()}" target="_blank" class="conf-cal-btn gcal">
        <img src="https://www.google.com/favicon.ico" width="16" height="16" alt=""> Google Calendar
      </a>
      <button onclick="baixarICS()" class="conf-cal-btn ical">
        📅 Baixar .ics
      </button>`;
  }
}
...
function gerarLinkGoogleCalendar() {
  const { date, time, barbeiro } = booking;
  if (!date || !time) return '#';

  const [dia, mes, ano] = date.split('/');
  const [hora, min]     = time.split(':');
  const dt = new Date(ano, mes - 1, dia, parseInt(hora), parseInt(min || 0));
  ...
  const servicos = cart.map(c => c.name).join(' + ');
  const titulo   = encodeURIComponent(`✂ Barbearia do Davi – ${servicos}`);
  const detalhes = encodeURIComponent(
    `Serviço: ${servicos}\nBarbeiro: ${barbeiro?.nome || '—'}\nLink para remarcar: ${location.origin}/historico`
  );
  const local    = encodeURIComponent('Vila Guará, Luziânia – GO');

  return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${titulo}&dates=${fmt(dt)}/${fmt(dtFim)}&details=${detalhes}&location=${local}`;
}
...
export function baixarICS() {
  ...
  const fmt = d => d.toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z';
  const uid = 'davi-' + Date.now() + '@barbearia-davi.com';
  const servicos = cart.map(c => c.name).join(' + ');

  const ics = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Barbearia do Davi//PT',
    'BEGIN:VEVENT',
    `UID:${uid}`,
    `DTSTAMP:${fmt(new Date())}`,
    `DTSTART:${fmt(dt)}`,
    `DTEND:${fmt(dtFim)}`,
    `SUMMARY:✂ Barbearia do Davi – ${servicos}`,
    `DESCRIPTION:Serviço: ${servicos}\\nBarbeiro: ${barbeiro?.nome || '—'}\\nRemar: ${location.origin}/historico`,
    'LOCATION:Vila Guará\\, Luziânia – GO',
    'END:VEVENT',
    'END:VCALENDAR',
  ].join('\r\n');
  ...
  a.href     = url;
  a.download = 'agendamento-barbearia-davi.ics';
  a.click();
...
export async function aplicarCupomCheckout() {
  ...
  try {
    const resultado = await validarCupom(codigo, totalBruto);
    cupomAplicado = resultado;

    if (msgEl) {
      msgEl.className = 'cupom-msg sucesso';
      const desc = resultado.tipo === 'percentual'
        ? `${resultado.valor}% de desconto`
        : `R$ ${resultado.valor.toFixed(2)} de desconto`;
      msgEl.textContent = `✅ Cupom aplicado! ${desc} → Total: R$ ${resultado.totalFinal.toFixed(2)}`;
    }

    // Atualiza total exibido no resumo do Pix
    const totalEl = document.getElementById('pixSumTotal');
    if (totalEl) totalEl.textContent = resultado.totalFinal.toFixed(2);

    // Mostra botão de remover
    const remBtn = document.getElementById('cupomRemBtn');
    ...
    cupomAplicado = null;
    if (msgEl) {
      msgEl.className = 'cupom-msg erro';
...
export function removerCupom() {
  ...
  if (msgEl)  { msgEl.textContent = ''; msgEl.className = 'cupom-msg'; }
  if (remBtn) remBtn.style.display = 'none';
  // Restaura total original
  const totalEl = document.getElementById('pixSumTotal');
  if (totalEl) totalEl.textContent = cart.reduce((s, c) => s + c.price, 0);
}
