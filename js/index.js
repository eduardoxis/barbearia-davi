...
export function renderGallery() {
  ...
  adminSettings.services.filter(s => !s.hidden).forEach(svc => {
    ...
    card.innerHTML = `
      ...
      <div class="gallery-img ${svc.bg}">${svc.icon}</div>
...
export function updateCartUI() {
  ...
  if (cart.length > 0) {
    ...
    if (badge) { badge.textContent = cart.length; badge.classList.add('fab-badge-show'); }
    fab.classList.remove('fab-pulse');
    void fab.offsetWidth;
    fab.classList.add('fab-pulse');
  } else {
    fab.classList.remove('fab-visible', 'fab-pulse');
    if (badge) badge.classList.remove('fab-badge-show');
  }
}
...
export function toggleCart() {
  ...
  if (fab) { fab.style.opacity = isOpen ? '0' : ''; fab.style.pointerEvents = isOpen ? 'none' : ''; }
}
