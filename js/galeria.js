...
/* ════════════════════════════════════════════
   DOM WIRING — conecta botões estáticos do HTML
   sem depender de onclick global (seguro em módulos ES6)
════════════════════════════════════════════ */
function _wireStaticButtons() {
  // Overlay da galeria — clicar fora fecha
  const ov = document.getElementById('overlayGaleria');
  if (ov) ov.addEventListener('click', e => { if (e.target === ov) fecharGaleria(); });

  // Botão X da galeria (id setado pelo patch)
  const closeGale = document.getElementById('galeBtnClose');
  if (closeGale) closeGale.addEventListener('click', fecharGaleria);

  // Lightbox close
  const lbClose = document.getElementById('galeLbCloseBtn');
  if (lbClose) lbClose.addEventListener('click', fecharLightboxGaleria);

  // Lightbox nav
  const lbPrev = document.getElementById('galeLbPrev');
  if (lbPrev) lbPrev.addEventListener('click', () => navLightbox(-1));

  const lbNext = document.getElementById('galeLbNext');
  if (lbNext) lbNext.addEventListener('click', () => navLightbox(1));

  // "Quero esse corte"
  const queroBtn = document.getElementById('galeLbQueroBtn');
  if (queroBtn) queroBtn.addEventListener('click', queroEsseCorte);

  // Lightbox swipe — id do container pode ser o próprio overlay
  const lbOv = document.getElementById('galeOverlayLb') || document.getElementById('galeOverlayLbInner');
  if (lbOv) {
    lbOv.addEventListener('touchstart', lbTouchStart, { passive: true });
    lbOv.addEventListener('touchend',   lbTouchEnd,   { passive: true });
  }

  // Modal de tags
  const tagCancel = document.getElementById('galeTagCancelBtn');
  const tagSave   = document.getElementById('galeTagSaveBtn');
  if (tagCancel) tagCancel.addEventListener('click', fecharEditarTags);
  if (tagSave)   tagSave.addEventListener('click', confirmarEditarTags);
}

// Executa quando o DOM estiver pronto
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', _wireStaticButtons);
} else {
  _wireStaticButtons();
}
